// index.js — robust register (modal -> selects -> auto submit), Arabic UI
...
async function finalizeRegistration(interaction, draft) {
  try {
    ...
    // Edit the ephemeral message (remove components)
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (command) {
        const context = { cfg, users: loadUsers, saveUsers };
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "reg_status_after") {
      ...
      // Keep the submit row if present (fallback)
      ...
      if (d.kind === "فصيل") {
        // Build faction row (keep submit as fallback)
        const factionRow =
          current.find(r => r.components?.[0]?.customId === "reg_faction_after") ||
    ...
    if (interaction.isButton()) {
      ...
      if (action === "approve" || action === "reject") {
        if (!hasPermission(interaction.member, action === "approve" ? "approve" : "reject")) {
        ...
        const logCh =
          interaction.client.channels.cache.get(ADMIN_LOG_CHANNEL_ID) ||
          (await interaction.client.channels.fetch?.(ADMIN_LOG_CHANNEL_ID).catch(() => null));
        if (logCh) logCh.send(`${approved ? "✅" : "⛔"} ${interaction.user.username} ${approved ? "قبل" : "رفض"} حساب <@${userId}>`);
      ...
      if (action === "promote") {
        ...
        const rankRow = new ActionRowBuilder().addComponents(
          ranks.map((rankName) =>
            new ButtonBuilder().setCustomId(`setrank_${userId}_${rankName}`).setLabel(rankName).setStyle(ButtonStyle.Secondary)
          )
        );
        return interaction.reply({
