...
module.exports = {
  ...
  async execute(interaction, { cfg, users, saveUsers, pushTx }) {
    ...
    const U = users();
    const A = U[uid];
    if (!A) return interaction.reply({ content:"لا يوجد حساب.", ephemeral:true });
    ...
    if (spent + total > (g.DAILY_WITHDRAW_LIMIT||Infinity))
      return interaction.reply({ content:`تجاوزت حد السحب اليومي (${g.DAILY_WITHDRAW_LIMIT}${g.CURRENCY_SYMBOL}).`, ephemeral:true });

    A.balance -= total;
    A._daily[key] = spent + total;
    saveUsers(U, interaction.guild);
    pushTx({ type:"withdraw", from:uid, amount, fee });
    return interaction.reply({ content:`💸 تم سحب ${amount}${g.CURRENCY_SYMBOL} (رسوم ${fee}).`, ephemeral:true });
  }
};
